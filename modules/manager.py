...
def get_bot_revenue(bot_id, start_date=None, end_date=None):
    conn = sqlite3.connect("data.db")
    cursor = conn.cursor()
    
    query = """
    SELECT SUM(CAST(json_extract(plano, '$.value') as REAL)) as total
    FROM PAYMENTS
    WHERE bot = ? AND status = 'finished'
    """
    params = [bot_id]
    
    if start_date and end_date:
        query += " AND trans_id >= ? AND trans_id <= ?"
...
def get_bot_revenue_by_period(bot_id, period):
    from datetime import datetime, timedelta
    ...
    if period == 'today':
        ...
        return get_bot_revenue(bot_id, None, None)
    else:
        days = int(period.replace('d', ''))
        start_date = now - timedelta(days=days)
        end_date = now
    
    return get_bot_revenue(bot_id, start_date.isoformat(), end_date.isoformat())